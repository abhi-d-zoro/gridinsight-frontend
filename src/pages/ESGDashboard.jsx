...
export default function ESGDashboard() {
  ...
  const fetchCarbonData = async () => {
    ...
    try {
      ...
      setMessage("Using fallback mock data (Backend integration in next sprint).");
      setCarbonData([
  ...
  // ================= SECURITY CHECK =================
  // Includes a temporary bypass for ASSET_MANAGER so your demo works smoothly!
  // It also handles Spring Boot's automatic 'ROLE_' prefix.
  if (
    role !== "ESG" &&
      ...
      <div style={{ padding: "32px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        {activeView === "DASHBOARD" && (
          <div style={gridStyle}>
            ...
            <div style={{ ...cardStyle, borderLeft: "4px solid #8b5cf6" }} onClick={() => setActiveView("CERTIFICATES")}>
              <h4 style={{ ...cardTitleStyle, color: "#a78bfa" }}>Green Certificates</h4>
              <p style={cardDescStyle}>Manage REC issuances and trading</p>
            </div>
          </div>
        )}
        ...
        {activeView === "CERTIFICATES" && (
          <div style={panelContainerStyle}>
            <button style={backButtonStyle} onClick={() => setActiveView("DASHBOARD")}>← Back to Dashboard</button>
            <h4 style={{ color: "#a78bfa", fontSize: "20px" }}>Green Certificates (REC)</h4>
            <p style={{ color: "#9ca3af" }}>REC generation and trading module is scheduled for backend integration in the next sprint.</p>
          </div>
        )}
